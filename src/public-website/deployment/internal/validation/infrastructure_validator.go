...
// WithHTTPClient allows injection of a custom HTTP client (useful for testing)
func (iv *InfrastructureValidator) WithHTTPClient(client HTTPClient) *InfrastructureValidator {
	...
	for key := range outputs {
		if strings.Contains(strings.ToLower(key), "health") || 
		   ...
		   strings.Contains(strings.ToLower(key), "url") {
			...
			// In real scenarios, these would be extracted from actual infrastructure outputs
...
// extractHealthCheckEndpointsFromMap is a testing-friendly version that accepts a plain map
func (iv *InfrastructureValidator) extractHealthCheckEndpointsFromMap(outputs map[string]string) map[string]string {
	endpoints := make(map[string]string)
...
func (ehc *EnvironmentHealthChecker) PerformHealthCheck(ctx context.Context, outputs map[string]interface{}) (*EnvironmentHealthReport, error) {
	...
	// Basic health check implementation for GREEN phase
	for component := range outputs {
		report.Components[component] = ComponentHealth{
			Name:      component,
