...
func testDomainWorkflow(t *testing.T, db *sql.DB, migrationDomain, validationDomain string) {
	...
	migrationsPath := filepath.Join(wd, "..", migrationDomain)
	validationPath := filepath.Join(wd)
	...
	// Validate schema
	validator := NewSchemaValidator(db, validationPath)
...
`
	...
	comparison := validator.compareSchemas("test", expectedTables, actualTables, []string{"idx_expected_table_title"}, []string{}, []string{}, []string{})

	assert.Contains(t, comparison.MissingTables, "expected_table")
	assert.Contains(t, comparison.ExtraTables, "invalid_test_table")
	assert.Contains(t, comparison.MissingIndexes, "idx_expected_table_title")
...
CREATE TABLE test_table2 (
	...
	validator := NewSchemaValidator(db, tempDir)
