        ...
        <div class="relative p-8 md:p-10 lg:p-12">
          <div class="flex flex-col lg:flex-row items-center gap-6 lg:gap-8">
            ...
            <div class="flex-1 text-center lg:text-left space-y-3">
              <h3 class="text-xl md:text-2xl font-semibold text-white">Stay Updated with News, Research & Events</h3>
            </div>
            ...
            <form @submit="handleSubmit" class="w-full max-w-md relative">
              ...
              <div class="flex flex-col sm:flex-row gap-3">
                <Input
                  ...
                  :class="
                    cn(
                      'flex-1 h-auto px-3 py-2 bg-white/10 text-white placeholder:text-white/60 focus:bg-white/20 border-white/20 focus:border-white/40 transition-all duration-200 rounded-sm border',
                ...
                <Button
                  type="submit"
                  :disabled="status === 'loading' || status === 'success'"
                  class="bg-white text-blue-600 hover:bg-gray-100 font-semibold px-6 py-2 rounded-sm border border-white hover:border-gray-100 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
                >
                  {{
                    status === 'loading'
                      ? 'Subscribing...'
...
const handleSubmit = async (e: Event) => {
  ...
  try {
    ...
    const subscriptionData: NewsletterSubscriptionData = {
      ...
      contentType: 'all', // Subscribe to all content types (news, research, and events)
    };
