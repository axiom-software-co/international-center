...
// TDD Cycle 8 RED Phase: Gateway Authentication Middleware Contract Tests
//
...
func TestPublicGatewayAuthentication_Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "public research endpoint should allow anonymous access",
			method:         "GET",
		...
		{
			name:           "public events endpoint should allow anonymous access",
			method:         "GET",
			path:           "/api/v1/events",
			headers:        map[string]string{},
			expectedStatus: http.StatusOK,
		...
		{
			name:           "non-existent routes should return 404",
			method:         "GET",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gateway, err := NewPublicGateway(ctx, "development")
			require.NoError(t, err, "Public gateway creation should succeed")
			require.NotNil(t, gateway, "Public gateway should not be nil")
...
func TestAdminGatewayAuthentication_Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "admin research management should require authentication",
			method:         "PUT",
			path:           "/admin/api/v1/research/123",
			headers:        map[string]string{"Content-Type": "application/json"},
			expectedStatus: http.StatusUnauthorized,
			expectAuth:     true,
			expectAudit:    true,
		...
		},
		{
			...
			method:         "GET",
			path:           "/admin/api/v1/services",
			headers:        map[string]string{"Authorization": "Bearer malformed_token"},
			expectedStatus: http.StatusUnauthorized,
			expectAuth:     true,
			expectAudit:    true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gateway, err := NewAdminGateway(ctx, "development")
			require.NoError(t, err, "Admin gateway creation should succeed")
			require.NotNil(t, gateway, "Admin gateway should not be nil")
			...
			// Contract: No caching headers should be set for admin interface
			if recorder.Code == http.StatusOK && tt.path != "/health" {
				cacheControl := recorder.Header().Get("Cache-Control")
				assert.Contains(t, cacheControl, "no-cache", "Admin responses should not be cached")
			}
...
func TestRoleBasedAccessControl_Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "admin should access audit endpoints",
			token:          "valid_admin_token",
			method:         "GET",
			path:           "/admin/api/v1/services/123/audit",
			expectedStatus: http.StatusOK,
			expectedRole:   "admin",
		},
		{
			name:           "content_editor should not access audit endpoints",
			token:          "valid_editor_token",
			method:         "GET",
			path:           "/admin/api/v1/services/123/audit",
			expectedStatus: http.StatusForbidden,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Contract: Permission denied responses should include reason
			if recorder.Code == http.StatusForbidden {
				assert.Contains(t, recorder.Body.String(), "insufficient permissions", "Permission denied message should be informative")
			}
		})
	}
}
...
func TestRateLimitingIntegration_Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "admin gateway should allow requests under limit",
			gatewayType:   "admin",
			requestCount:  10, // Under 100/min limit
			expectedLimit: false,
			limitType:     "user",
		},
	}
...
func TestSecurityHeadersIntegration_Contract(t *testing.T) {
	...
	gatewayTypes := []struct {
		...
		{"public", func(ctx context.Context, env string) (http.Handler, error) { return NewPublicGateway(ctx, env) }},
		{"admin", func(ctx context.Context, env string) (http.Handler, error) { return NewAdminGateway(ctx, env) }},
	}
	...
	for _, gt := range gatewayTypes {
		t.Run(gt.name+"_gateway_security_headers", func(t *testing.T) {
			...
			gateway, err := gt.gateway(ctx, "development")
			require.NoError(t, err)
...
func TestDAPRMiddlewareIntegration_Contract(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "production environment should use production DAPR services",
			environment:   "production", 
			expectedDAPR:  true,
			expectedVault: "https://vault.axiomcloud.dev",
			expectedRedis: "redis-prod.upstash.io:6379",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config, err := NewGatewayDAPRConfiguration(tt.environment)
...
func TestEnvironmentSpecificConfiguration_Contract(t *testing.T) {
	timeout := 5 * time.Second
	_, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	...
	environments := []struct {
		...
		expectedAuditLevel  string
	}{
		{
			name:               "development should have relaxed settings",
			environment:        "development",
			expectedTimeout:    30 * time.Second,
			expectedRateLimit:  1000,
			expectedAuditLevel: "info",
		},
		{
			name:               "staging should have moderate settings", 
			environment:        "staging",
			expectedTimeout:    45 * time.Second,
			expectedRateLimit:  500,
			expectedAuditLevel: "warning",
		},
		{
			name:               "production should have strict settings",
			environment:        "production",
			expectedTimeout:    60 * time.Second,
			expectedRateLimit:  100,
			expectedAuditLevel: "error",
		},
	}

	for _, tt := range environments {
		t.Run(tt.name, func(t *testing.T) {
			// Contract: Gateway configuration should be environment-specific
			publicConfig := NewPublicGatewayConfiguration()
			publicConfig.Environment = tt.environment
			
			adminConfig := NewAdminGatewayConfiguration() 
			adminConfig.Environment = tt.environment

			// Apply environment-specific overrides
			err := ApplyEnvironmentConfiguration(publicConfig, tt.environment)
			require.NoError(t, err, "Environment configuration should apply successfully")
			
			err = ApplyEnvironmentConfiguration(adminConfig, tt.environment)
			require.NoError(t, err, "Environment configuration should apply successfully")

			// Contract: Timeout configuration should match environment
			assert.Equal(t, tt.expectedTimeout, adminConfig.Timeouts.RequestTimeout, "Request timeout should match environment")

			// Contract: Rate limiting should be environment-appropriate
			if publicConfig.RateLimit.Enabled {
				// Production should have stricter limits
				if tt.environment == "production" {
					assert.LessOrEqual(t, publicConfig.RateLimit.RequestsPerMinute, 1000, "Production should have stricter rate limits")
				}
			}

			// Contract: Audit configuration should match environment requirements
			if adminConfig.Observability.Enabled {
				// Verify audit level is appropriate for environment
				assert.NotEmpty(t, tt.expectedAuditLevel, "Audit level should be configured")
			}
		})
	}
}