...
func (vm *VolumeManager) CreateVolumeConfigurations() (map[string]*VolumeConfiguration, error) {
	...
	for _, appConfig := range vm.config.Applications {
		configurations[appConfig.Name] = vm.createApplicationVolumes(appConfig.Name, appConfig)
	}
...
func (vm *VolumeManager) needsPersistentStorage(serviceName string) bool {
	...
	for _, appConfig := range vm.config.Applications {
		if appConfig.Name == serviceName && appConfig.PersistentStorage.Enabled {
			return true
		}
	}
...
func (vm *VolumeManager) createAzureFilesVolume(volumeName string, volumeMount VolumeMount) (pulumi.Resource, error) {
	...
	fileShare, err := storage.NewFileShare(vm.ctx, volumeName, &storage.FileShareArgs{
		...
		AccessTier:         storage.ShareAccessTierHot,
		EnabledProtocols:   storage.EnabledProtocolsSMB,
	})
	
	if err != nil {
		return nil, fmt.Errorf("failed to create Azure file share %s: %w", volumeName, err)
	}
