...
import (
	...
	"github.com/pulumi/pulumi-azure-native/sdk/v2/go/azurenative/keyvault"
...
func NewSecretsManager(ctx *pulumi.Context, cfg *config.DeploymentConfig) *SecretsManager {
	provider := sm.determineSecretProvider(cfg)
...
func (sm *SecretsManager) CreateSecretConfigurations() (map[string]*SecretConfiguration, error) {
	...
	// Application secrets
	for _, appConfig := range sm.config.Applications {
		configurations[appConfig.Name] = sm.createApplicationSecrets(appConfig.Name, appConfig)
	}
...
func (sm *SecretsManager) createApplicationSecrets(serviceName string, appConfig config.ApplicationConfig) *SecretConfiguration {
	...
	// Database connection password (if service has its own DB)
	if appConfig.Database.Enabled {
		secrets["db-password"] = SecretMetadata{
			Name:        fmt.Sprintf("%s-db-password", serviceName),
			Description: fmt.Sprintf("%s database password", serviceName),
			SecretType:  SecretTypeDatabase,
			Provider:    sm.provider,
			Tags: map[string]string{
				"service":     serviceName,
	...
	if appConfig.TLS.Enabled {
		secrets["tls-cert"] = SecretMetadata{
			Name:        fmt.Sprintf("%s-tls-cert", serviceName),
			Description: fmt.Sprintf("%s TLS certificate", serviceName),
			SecretType:  SecretTypeTLS,
			Provider:    sm.provider,
			Tags: map[string]string{
				"service":     serviceName,
...
func (sm *SecretsManager) createAzureKeyVaultSecret(secretName string, secretData map[string]string, metadata SecretMetadata) (pulumi.Resource, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create Azure Key Vault secret %s: %w", secretName, err)
	}
