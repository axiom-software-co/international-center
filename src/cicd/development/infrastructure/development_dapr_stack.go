...
func (ds *DaprStack) Deploy(ctx context.Context) (sharedinfra.DaprDeployment, error) {
	...
	// Deployment outputs are handled through stack outputs
...
`
	...
	// Dapr configuration with middleware pipeline
	daprConfigYAML := `
apiVersion: dapr.io/v1alpha1
kind: Configuration
metadata:
...
`
	...
	// Create component files directory if it doesn't exist
	componentDir := "/tmp/dapr-components"
	err := os.MkdirAll(componentDir, 0755)
	if err != nil {
		return fmt.Errorf("failed to create Dapr components directory: %w", err)
	}

	// Write component YAML files
	components := map[string]string{
		"pubsub.yaml":        pubsubComponentYAML,
		"statestore.yaml":    stateStoreComponentYAML,
		"secrets.yaml":       secretStoreComponentYAML,
		"blob-storage.yaml":  blobStorageComponentYAML,
		"config.yaml":        daprConfigYAML,
	}

	for filename, content := range components {
		filePath := filepath.Join(componentDir, filename)
		if err := os.WriteFile(filePath, []byte(content), 0644); err != nil {
			return fmt.Errorf("failed to write Dapr component file %s: %w", filename, err)
		}
		fmt.Printf("Generated Dapr component: %s\n", filePath)
	}

	// Copy components to the mounted volume using Docker CP
	// The components will be available to sidecars via the mounted volume
	copyCmd := fmt.Sprintf("docker cp %s/. dapr-components-volume:/", componentDir)
	if err := ds.executeBashCommand(copyCmd); err != nil {
		// For development environment, components can also be mounted directly
		fmt.Printf("Note: Components generated at %s for development testing\n", componentDir)
	}
...
func (ds *DaprStack) executeBashCommand(command string) error {
