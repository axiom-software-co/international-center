...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		// Deploy Service Stack using factory (backend services with Dapr sidecars)
		serviceStack := factory.CreateServiceStack(ctx, pulumiConfig, environment)
		serviceDeployment, err := serviceStack.Deploy(context.Background())
		if err != nil {
		...
		// Service endpoints from deployed service stack
		...
		ctx.Export("CONTENT_API_URL", pulumi.String(serviceEndpoints["content-api"]))
		ctx.Export("SERVICES_API_URL", pulumi.String(serviceEndpoints["services-api"]))
		ctx.Export("PUBLIC_GATEWAY_URL", pulumi.String(serviceEndpoints["public-gateway"]))
		ctx.Export("ADMIN_GATEWAY_URL", pulumi.String(serviceEndpoints["admin-gateway"]))
		...
		// Initialize Vault secrets after deployment - temporarily disabled for deployment stability
		// TODO: Add retry logic and proper readiness checks before initializing secrets
		...
		// Create Vault policies - temporarily disabled for deployment stability
		// TODO: Add retry logic and proper readiness checks before creating policies
		// err = vaultStack.CreatePolicies(context.Background(), vaultDeployment)
		// if err != nil {
		// 	return fmt.Errorf("failed to create Vault policies: %w", err)
		// }

		// Configure observability data sources - temporarily disabled for deployment stability
		// TODO: Add retry logic and proper readiness checks before configuring data sources
		...
		// Create storage containers - temporarily disabled for deployment stability
		// TODO: Add retry logic and proper readiness checks before creating storage containers
		...
		if err := serviceStack.ValidateDeployment(context.Background(), serviceDeployment); err != nil {
