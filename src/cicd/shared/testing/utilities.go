...
// ValidateContainerHealth validates that a container is running and healthy
func ValidateContainerHealth(t *testing.T, containerName string, healthEndpoint string) bool {
	t.Helper()
	...
	if healthEndpoint == "" {
		t.Logf("No health endpoint specified for container %s", containerName)
		return false
	}
	
	req, err := http.NewRequestWithContext(ctx, "GET", healthEndpoint, nil)
	if err != nil {
		t.Logf("Failed to create health check request for %s: %v", containerName, err)
		return false
	}
	...
	resp, err := client.Do(req)
	if err != nil {
		t.Logf("Container %s health check failed: %v", containerName, err)
		return false
	}
	defer resp.Body.Close()
	...
	if healthy {
		t.Logf("✅ Container %s is healthy (status: %d)", containerName, resp.StatusCode)
	} else {
		t.Logf("⚠️ Container %s unhealthy (status: %d)", containerName, resp.StatusCode)
	}
	
	return healthy
}

// ValidateContainerConnectivity validates TCP connectivity to a container service
func ValidateContainerConnectivity(t *testing.T, serviceName string, address string, timeout time.Duration) bool {
	t.Helper()
	
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	
	conn, err := ConnectWithTimeout(ctx, "tcp", address, timeout)
	if err != nil {
		t.Logf("Container %s connectivity failed at %s: %v", serviceName, address, err)
		return false
	}
	defer conn.Close()
	
	t.Logf("✅ Container %s connectivity verified at %s", serviceName, address)
	return true
}

// ValidateDatabaseContainerReadiness validates PostgreSQL container is ready for migrations
func ValidateDatabaseContainerReadiness(t *testing.T, connectionString string) bool {
	t.Helper()
	...
	// Test migration readiness - check if we can create schemas
	var canCreate bool
	err = db.QueryRowContext(ctx, "SELECT has_database_privilege(current_database(), 'CREATE')").Scan(&canCreate)
	if err != nil {
		t.Logf("Failed to check database privileges: %v", err)
		return false
	}
	
	if !canCreate {
		t.Log("Database does not allow schema creation")
		return false
	}
	
	// Test basic table operations capability
	testTable := "deployment_validation_test_" + fmt.Sprintf("%d", time.Now().Unix())
	_, err = db.ExecContext(ctx, fmt.Sprintf("CREATE TEMP TABLE %s (id SERIAL PRIMARY KEY)", testTable))
	if err != nil {
		t.Logf("Failed to create test table: %v", err)
		return false
	}
	
	t.Log("✅ Database container is ready for migrations")
	return true
}

// ValidateRedisContainerReadiness validates Redis container for state store and pub/sub
func ValidateRedisContainerReadiness(t *testing.T, addr string, password string) bool {
	t.Helper()
	...
	testKey := fmt.Sprintf("deployment_test_%d", time.Now().Unix())
	err = client.Set(ctx, testKey, "test_value", time.Minute).Err()
	if err != nil {
		t.Logf("Redis SET operation failed: %v", err)
		return false
	}
	
	value, err := client.Get(ctx, testKey).Result()
	if err != nil {
		t.Logf("Redis GET operation failed: %v", err)
		return false
	}
	
	if value != "test_value" {
		t.Logf("Redis returned incorrect value: expected 'test_value', got '%s'", value)
		return false
	}
	
	// Test pub/sub capability
	pubsub := client.Subscribe(ctx, "deployment_test_channel")
	defer pubsub.Close()
	
	err = client.Publish(ctx, "deployment_test_channel", "test_message").Err()
	if err != nil {
		t.Logf("Redis pub/sub publish failed: %v", err)
		return false
	}
	...
	t.Log("✅ Redis container is ready for state store and pub/sub")
	return true
}

// ValidateVaultContainerReadiness validates Vault container initialization and secret store capability
func ValidateVaultContainerReadiness(t *testing.T, vaultAddr string) bool {
	t.Helper()
	...
	resp, err := MakeHTTPRequest(ctx, "GET", vaultAddr+"/v1/sys/health", nil)
	if err != nil {
		t.Logf("Vault health check failed: %v", err)
		return false
	}
	defer resp.Body.Close()
	
	// Vault health endpoint returns various status codes for healthy states
	if resp.StatusCode >= 500 {
		t.Logf("Vault health check failed with status: %d", resp.StatusCode)
		return false
	}
	
	// Test secret engines availability
	resp, err = MakeHTTPRequest(ctx, "GET", vaultAddr+"/v1/sys/mounts", nil)
	if err != nil {
		t.Logf("Vault mounts check failed: %v", err)
		return false
	}
	defer resp.Body.Close()
	
	if resp.StatusCode >= 500 {
		t.Logf("Vault mounts not accessible: %d", resp.StatusCode)
		return false
	}
	
	t.Log("✅ Vault container is ready for secret storage")
	return true
}

// ValidateAzuriteContainerReadiness validates Azurite blob storage emulator
func ValidateAzuriteContainerReadiness(t *testing.T, azuriteEndpoint string) bool {
	t.Helper()
	...
	resp, err := MakeHTTPRequest(ctx, "GET", azuriteEndpoint+"/devstoreaccount1?comp=list", nil)
	if err != nil {
	...
	if resp.StatusCode >= 500 {
		t.Logf("Azurite blob service failed: %d", resp.StatusCode)
		return false
	}
	
	// Test container operations capability
	resp, err = MakeHTTPRequest(ctx, "GET", azuriteEndpoint+"/devstoreaccount1/test-container?restype=container", nil)
	if err != nil {
		t.Logf("Azurite container operations test failed: %v", err)
		return false
	}
	defer resp.Body.Close()
	
	// 404 is acceptable for non-existent container, indicates API is working
	if resp.StatusCode >= 500 {
		t.Logf("Azurite container operations not available: %d", resp.StatusCode)
		return false
	}
	
	t.Log("✅ Azurite container is ready for blob storage operations")
	return true
}

// ValidateDaprControlPlaneReadiness validates Dapr control plane container
func ValidateDaprControlPlaneReadiness(t *testing.T, daprEndpoint string) bool {
	t.Helper()
	...
	resp, err := MakeHTTPRequest(ctx, "GET", daprEndpoint+"/v1.0/healthz", nil)
	if err != nil {
		t.Logf("Dapr health check failed: %v", err)
		return false
	}
	defer resp.Body.Close()
	
	if resp.StatusCode != http.StatusOK {
		t.Logf("Dapr health check failed with status: %d", resp.StatusCode)
		return false
	}
	
	// Test Dapr metadata endpoint for service discovery
	resp, err = MakeHTTPRequest(ctx, "GET", daprEndpoint+"/v1.0/metadata", nil)
	if err != nil {
		t.Logf("Dapr metadata check failed: %v", err)
		return false
	}
	defer resp.Body.Close()
	
	if resp.StatusCode != http.StatusOK {
		t.Logf("Dapr metadata endpoint failed: %d", resp.StatusCode)
		return false
	}
	
	t.Log("✅ Dapr control plane is ready for service discovery")
	return true
}

// ValidateCompleteInfrastructureReadiness performs comprehensive infrastructure validation
func ValidateCompleteInfrastructureReadiness(t *testing.T, env *Environment) bool {
	t.Helper()
	
	allHealthy := true
	
	t.Run("Complete_Infrastructure_Validation", func(t *testing.T) {
		// Validate database readiness
		if env.DatabaseURL != "" {
			if !ValidateDatabaseContainerReadiness(t, env.DatabaseURL) {
				allHealthy = false
			}
		}
		
		// Validate Redis readiness
		if env.RedisAddr != "" {
			if !ValidateRedisContainerReadiness(t, env.RedisAddr, env.RedisPassword) {
				allHealthy = false
			}
		}
		...
		if env.VaultAddr != "" {
			if !ValidateVaultContainerReadiness(t, env.VaultAddr) {
				allHealthy = false
			}
		}
		
		// Validate Azurite readiness
		azuriteEndpoint := GetEnvVar("AZURITE_BLOB_ENDPOINT", "http://localhost:10000")
		if !ValidateAzuriteContainerReadiness(t, azuriteEndpoint) {
			allHealthy = false
		}
		
		// Validate Dapr control plane readiness
		daprPort := GetEnvVar("DAPR_HTTP_PORT", "3500")
		daprEndpoint := fmt.Sprintf("http://localhost:%s", daprPort)
		if !ValidateDaprControlPlaneReadiness(t, daprEndpoint) {
			allHealthy = false
		}
	})
	
	if allHealthy {
		t.Log("✅ Complete infrastructure is ready for application services")
	} else {
		t.Log("⚠️ Infrastructure readiness issues detected")
	}
