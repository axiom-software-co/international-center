...
func (nm *NetworkManager) createNetwork(spec NetworkSpec) (*docker.Network, error) {
	...
	if spec.IPAM.Driver != "" {
		...
		_ = spec.IPAM // Prevent unused variable error
	}
	...
	// TODO: Network labels disabled - Docker SDK v4 compatibility issue
	// Add labels
	labels := nm.buildNetworkLabels(spec.Labels)
	if len(labels) > 0 {
		// Labels functionality temporarily disabled due to Docker SDK v4 incompatibility
		// pulumi.StringMap does not implement docker.NetworkLabelArrayInput
		// This will need to be re-implemented when Docker SDK supports labels properly
...
func (nm *NetworkManager) CreateServiceNetworkConfiguration() *NetworkServiceConfiguration {
...
// NetworkServiceConfiguration represents the complete service network setup
