...
func TestDaprServiceInvocation(t *testing.T) {
	...
	t.Run("dapr sidecar connectivity", func(t *testing.T) {
		...
		servicesApiDaprPort := requireEnv(t, "SERVICES_API_DAPR_HTTP_PORT")
		
		client := &http.Client{Timeout: 15 * time.Second}
		
		// Test services-api Dapr sidecar
		resp, err := client.Get(fmt.Sprintf("http://localhost:%s/v1.0/healthz", servicesApiDaprPort))
		if err == nil {
			defer resp.Body.Close()
			assert.Contains(t, []int{http.StatusOK, http.StatusNoContent}, resp.StatusCode, "Services API Dapr sidecar should be healthy")
		} else {
	...
	t.Run("service invocation through dapr", func(t *testing.T) {
		...
		resp, err := client.Get(fmt.Sprintf("http://localhost:%s/v1.0/invoke/services-api/method/health", servicesApiDaprPort))
		if err == nil {
			defer resp.Body.Close()
